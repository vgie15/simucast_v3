...
from .config import (
    MAX_UPLOAD_BYTES, MAX_UPLOAD_ROWS, GUEST_MODEL_LIMIT,
    CORS_ORIGINS, DATABASE_URL, _DATABASE_URL_FROM_ENV,
    _AI_MODEL_FAST, _AI_MODEL_DEEP,
)
from .db import engine, Base, SessionLocal, db, _ensure_schema, _try_init_at_startup, is_db_ready
from .models import (
    Dataset, User, UserSession, DatasetStage,
    Analysis, Model, ActivityLog, AIResponse,
)
...
def _schema_guard():
    ...
    if is_db_ready() or request.path == "/api/health":
        return None
    try:
        _ensure_schema()
